...
double PositionCalc(vector<unsigned int> ENCDAT, int check, ofstream& posno, ofstream& angno, vector<double>& data, double& a, double& b, double& c)
{
	float gearRatio[3];
	double Position[3];
	double x, y, z;
...
int main()
{
	...
	while (stop < 10000)
	{
		...
		PositionCalc(ENCDAT1, check, posno, angno, data1,x1,y1,z1);
		angno << "     ";
		posno << "     ";
		PositionCalc(ENCDAT2, check, posno, angno, data2,x2,y2,z2);
		angno << " \n";
		posno << " \n ";
